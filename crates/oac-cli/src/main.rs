...
use oac_core::{app_paths, adapter, manager, models::*, scanner, service, store::Store};
use serde::Serialize;

#[derive(Parser)]
#[command(
    name = "oac",
    about = "Open Agent Config — manage your AI agent extensions",
    version
)]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    Serve {
        ...
        /// stored at ~/.open-agent-config/web-token (mode 0600). Prefer this over
...
fn main() -> Result<()> {
    ...
    {
        let data_dir = app_paths::prepare_data_dir()?;
        let effective_token = resolve_serve_token(token, no_token, &data_dir);

        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(oac_web::serve(oac_web::ServeOptions {
            port,
            host,
...
/// Resolve the web access token for `oac serve`.
///
/// Secure-by-default: with neither `--token` nor `--no-token`, a persistent
/// token is loaded (or generated) from `~/.open-agent-config/web-token` and enforced
...
/// Load the persisted token from `~/.open-agent-config/web-token`, or create one.
///
...
fn cmd_audit(
    ...
    sorted.sort_by_key(|group| group.trust_score);
...
mod cli_json_tests {
    ...
    fn parse_list_json_flag() {
        let cli = Cli::try_parse_from(["oac", "list", "--json"]).unwrap();
    ...
    fn parse_list_agents_json_flag() {
        let cli = Cli::try_parse_from(["oac", "list", "agents", "--json"]).unwrap();
    ...
    fn parse_info_json_flag() {
        let cli = Cli::try_parse_from(["oac", "info", "demo", "--json"]).unwrap();
