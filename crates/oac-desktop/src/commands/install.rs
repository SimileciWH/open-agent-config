...
use oac_core::{OacError, deployer, manager, marketplace, models::*, scanner, service};
use tauri::State;
...
fn enabled_detected_agents(
    store: &oac_core::store::Store,
    adapters: &[Box<dyn oac_core::adapter::AgentAdapter>],
) -> Vec<String> {
    adapters
        .iter()
        .filter(|a| {
...
pub async fn install_from_local(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let pack = git_source
            .url
            .as_deref()
            .and_then(oac_core::scanner::extract_pack_from_url);
        {
...
pub async fn install_from_git(
    ...
    let adapters = state.adapters.clone();
    tauri::async_runtime::spawn_blocking(move || {
        oac_core::sanitize::validate_git_url(&url)
            .map_err(|e| OacError::Validation(e.to_string()))?;
        let (target_dir, agent_name) = if let Some(ref agent) = target_agent {
            let a = adapters
                .iter()
                .find(|a| a.name() == agent.as_str())
                .ok_or_else(|| OacError::NotFound(format!("Agent '{}' not found", agent)))?;
            ...
            let a = adapters
                .iter()
                .find(|a| a.detect())
                .ok_or_else(|| OacError::Internal("No detected agent found".into()))?;
...
pub async fn scan_git_repo(
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<ScanResult, OacError> {
            ...
            1 => {
                ...
                let agents = if target_agents.is_empty() {
                    vec![
                        enabled_detected_agents(&store_clone.lock(), &adapters)
                            .into_iter()
                            .next()
                            .ok_or_else(|| OacError::NotFound("No detected agent found".into()))?,
                    ]
                } else {
                    target_agents
                };
                ...
                for agent_name in &agents {
                    let a = adapters
                        ...
                        .find(|a| a.name() == agent_name.as_str())
                        .ok_or_else(|| {
                            OacError::NotFound(format!("Agent '{}' not found", agent_name))
                        })?;
                ...
                Ok(ScanResult::Installed {
                    result: last_result
                        .ok_or_else(|| OacError::Internal("No install results produced".into()))?,
                })
            }
            _ => {
...
pub async fn install_scanned_skills(
    ...
    target_agents: Vec<String>,
    target_scope: ConfigScope,
) -> Result<Vec<manager::InstallResult>, OacError> {
    let pending = {
        let mut clones = state.pending_clones.lock();
        clones
            .remove(&clone_id)
            .ok_or_else(|| OacError::Internal("Clone session expired. Please try again.".into()))?
    };

    let store_clone = state.store.clone();
    let adapters = state.adapters.clone();

    tauri::async_runtime::spawn_blocking(move || -> Result<Vec<manager::InstallResult>, OacError> {
        ...
        {
            let store = store_clone.lock();
            let install_pack = oac_core::scanner::extract_pack_from_url(&pending.url);
...
pub async fn install_new_repo_skills(
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<Vec<manager::InstallResult>, OacError> {
        ...
        {
            let store = store_clone.lock();
            let install_pack = oac_core::scanner::extract_pack_from_url(&url);
            let mut synced_skills = std::collections::HashSet::new();
            for (_agent_name, sid, result) in &results {
                if !synced_skills.insert(result.name.clone()) {
                    let meta = InstallMeta {
...
pub fn get_cli_with_children(
    ...
    cli_id: String,
) -> Result<(Extension, Vec<Extension>), OacError> {
    let store = state.store.lock();
    let cli = store
        .get_extension(&cli_id)?
        .ok_or_else(|| OacError::NotFound(format!("CLI not found: {}", cli_id)))?;
    let children = store.get_child_skills(&cli_id)?;
    Ok((cli, children))
}

#[tauri::command]
pub async fn list_cli_marketplace() -> Result<Vec<marketplace::MarketplaceItem>, OacError> {
