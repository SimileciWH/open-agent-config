use super::AppState;
use oac_core::{OacError, manager, marketplace, models::*, service};
use tauri::State;
...
pub async fn search_marketplace(
    ...
    kind: String,
    limit: Option<usize>,
) -> Result<Vec<marketplace::MarketplaceItem>, OacError> {
...
#[tauri::command]
pub async fn fetch_cli_readme(source: String) -> Result<String, OacError> {
...
pub async fn fetch_skill_audit(
    source: String,
    skill_id: String,
) -> Result<Option<marketplace::SkillAuditInfo>, OacError> {
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<manager::InstallResult, OacError> {
        let (target_dir, agent_name) = if let Some(ref agent) = target_agent {
            let a = adapters
                ...
                .ok_or_else(|| OacError::Internal(format!("Agent '{}' not found", agent)))?;
