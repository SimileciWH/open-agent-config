...
#[tauri::command]
pub fn list_projects(state: State<AppState>) -> Result<Vec<Project>, OacError> {
...
#[tauri::command]
pub fn add_project(state: State<AppState>, path: String) -> Result<Project, OacError> {
    // Canonicalize to prevent duplicates via symlinks/relative paths
    let project_path = std::path::Path::new(&path)
        .canonicalize()
        .map_err(|e| OacError::CommandFailed(format!("Invalid path: {}", e)))?;
    ...
    if existing.iter().any(|p| p.path == path) {
        return Err(OacError::Conflict("Project already added".into()));
    }
...
#[tauri::command]
pub fn remove_project(state: State<AppState>, id: String) -> Result<(), OacError> {
    let store = state.store.lock();
...
pub fn count_project_extensions(
    ...
    path: String,
) -> Result<ProjectExtensionCounts, OacError> {
    let p = std::path::Path::new(&path);
    Ok(core_count_project_extensions(p, &state.adapters))
}

#[tauri::command]
pub fn discover_projects(root_path: String) -> Result<Vec<DiscoveredProject>, OacError> {
