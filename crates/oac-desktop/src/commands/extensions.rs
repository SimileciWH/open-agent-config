...
use oac_core::service::ExtensionContent;
use oac_core::{OacError, manager, models::*, scanner, service};
...
#[tauri::command]
pub async fn delete_extension(state: State<'_, AppState>, id: String) -> Result<(), OacError> {
    let store = state.store.clone();
...
#[tauri::command]
pub fn uninstall_cli_binary(binary_path: String) -> Result<(), OacError> {
...
pub fn list_skill_files(
    ...
    path: String,
) -> Result<Vec<FileEntry>, OacError> {
    let root = std::path::Path::new(&path);
    if !root.is_dir() {
        return Err(OacError::Validation("Path is not a directory".into()));
    }
...
#[tauri::command]
pub fn open_in_system(_state: State<AppState>, path: String) -> Result<(), OacError> {
    let file_path = std::path::Path::new(&path);
    if !file_path.exists() {
        return Err(OacError::NotFound("Path does not exist".into()));
    }
    ...
    #[cfg(target_os = "macos")]
    {
        std::process::Command::new("open")
            .arg(&path)
            .spawn()
            .map_err(|e| OacError::CommandFailed(format!("Failed to open: {}", e)))?;
    }
...
#[tauri::command]
pub fn reveal_in_file_manager(_state: State<AppState>, path: String) -> Result<(), OacError> {
    let file_path = std::path::Path::new(&path);
    if !file_path.exists() {
        return Err(OacError::NotFound("Path does not exist".into()));
    }
    #[cfg(target_os = "macos")]
    {
        std::process::Command::new("open")
            ...
            .arg(&path)
            .spawn()
            .map_err(|e| OacError::CommandFailed(format!("Failed to reveal: {}", e)))?;
    }
    #[cfg(target_os = "windows")]
    {
        std::process::Command::new("explorer")
...
pub fn get_extension_content(
    state: State<AppState>,
    id: String,
) -> Result<ExtensionContent, OacError> {
    service::get_extension_content(&state.store, &state.adapters, &id)
}

#[tauri::command]
pub async fn scan_and_sync(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<usize, OacError> {
    let store = state.store.clone();
    let adapters = state.adapters.clone();
    ...
    if !unlinked.is_empty() {
        ...
        tauri::async_runtime::spawn_blocking(move || {
            ...
            for name in &unique_names {
                if let Ok(results) = oac_core::marketplace::search_skills(name, 5) {
                    let exact: Vec<_> = results.iter().filter(|r| r.name.eq_ignore_ascii_case(name)).collect();
                    if exact.len() == 1 {
                        let item = exact[0];
                        let git_url = oac_core::marketplace::git_url_for_source(&item.source);
...
pub fn get_cached_update_statuses(
    state: State<AppState>,
) -> Result<Vec<(String, UpdateStatus)>, OacError> {
    let store = state.store.lock();
...
pub async fn check_updates(
    state: State<'_, AppState>,
) -> Result<CheckUpdatesResult, OacError> {
    let store_clone = state.store.clone();

    tauri::async_runtime::spawn_blocking(move || -> Result<CheckUpdatesResult, OacError> {
        ...
        let mut new_skills: Vec<NewRepoSkill> = Vec::new();
        {
            ...
            for (url, indices) in &url_to_indices {
                ...
                for &idx in indices {
                    ...
                    {
                        eprintln!(
                            "[oac] Skill '{}' no longer exists in repository",
                            name
                        );
                ...
                let pack = oac_core::scanner::extract_pack_from_url(url);
...
pub async fn update_extension(
    state: State<'_, AppState>,
    id: String,
) -> Result<manager::InstallResult, OacError> {
    let store_clone = state.store.clone();

    tauri::async_runtime::spawn_blocking(move || -> Result<manager::InstallResult, OacError> {
        let (ext, install_meta) = {
            ...
            let meta = ext.install_meta.clone().ok_or_else(|| {
                OacError::NotFound("Extension has no install metadata — cannot update".into())
            })?;
            match meta.install_type.as_str() {
        ...
        let url = install_meta
            ...
            .ok_or_else(|| OacError::NotFound("Extension has no remote URL".into()))?;

        // Clone the repo once
        let temp = tempfile::tempdir().map_err(|e| OacError::Internal(e.to_string()))?;
        ...
        let skill_source = match manager::find_skill_in_repo(&clone_dir, skill_name) {
            ...
            None => {
                eprintln!(
                    "[oac] Skill '{}' no longer exists in repository — skipping update",
                    skill_name
                );
        ...
        for sibling in &all_siblings {
            let source_path = sibling
                .source_path
                .as_deref()
                .ok_or_else(|| OacError::Internal("Sibling extension has no source_path".into()))?;
            let skill_dir = std::path::Path::new(source_path).parent().ok_or_else(|| {
                OacError::Internal("Cannot determine skill directory from source path".into())
            })?;
            if !updated_dirs.insert(skill_dir.to_string_lossy().to_string()) {
                continue;
            }
            oac_core::deployer::deploy_skill(
                &skill_source,
