...
#[tauri::command]
pub fn get_dashboard_stats(state: State<AppState>) -> Result<DashboardStats, OacError> {
...
#[tauri::command]
pub fn update_tags(state: State<AppState>, id: String, tags: Vec<String>) -> Result<(), OacError> {
    let store = state.store.lock();
...
#[tauri::command]
pub fn get_all_tags(state: State<AppState>) -> Result<Vec<String>, OacError> {
    let store = state.store.lock();
    store.get_all_tags()
}
...
#[tauri::command]
pub fn get_all_packs(state: State<AppState>) -> Result<Vec<String>, OacError> {
    let store = state.store.lock();
    store.get_all_packs()
}
...
pub fn toggle_by_pack(
    ...
    for id in &ids {
        oac_core::manager::toggle_extension(&store, id, enabled)?;
    }
    Ok(ids)
}
...
pub fn read_config_file_preview(
    ...
    if !file_path.exists() {
        return Err(OacError::NotFound("File not found".into()));
    }

    // OS gates whether the user can read this file; OAC does not impose its
...
pub fn add_custom_config_path(
    ...
    let resolved = oac_core::sanitize::resolve_and_validate_config_path(&path)?;
...
#[tauri::command]
pub fn remove_custom_config_path(state: State<AppState>, id: i64) -> Result<(), OacError> {
    let store = state.store.lock();
