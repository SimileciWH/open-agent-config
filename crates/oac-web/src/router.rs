...
pub struct ApiError(StatusCode, OacError);
...
where
    ...
    F: FnOnce() -> std::result::Result<T, OacError> + Send + 'static,
{
    let result = tokio::task::spawn_blocking(f)
        .await
        .map_err(|e| ApiError::from(OacError::Internal(e.to_string())))?;
    Ok(Json(result?))
}

impl ApiError {
    pub fn not_found(msg: &str) -> Self {
        Self(StatusCode::NOT_FOUND, OacError::NotFound(msg.into()))
    }

    pub fn forbidden(msg: &str) -> Self {
        Self(StatusCode::FORBIDDEN, OacError::PermissionDenied(msg.into()))
    }
}
...
impl From<OacError> for ApiError {
    fn from(e: OacError) -> Self {
        let status = match &e {
            OacError::NotFound(_) => StatusCode::NOT_FOUND,
            OacError::Network(_) => StatusCode::BAD_GATEWAY,
            OacError::PermissionDenied(_) => StatusCode::FORBIDDEN,
            OacError::ConfigCorrupted(_) => StatusCode::INTERNAL_SERVER_ERROR,
            OacError::Conflict(_) => StatusCode::CONFLICT,
            OacError::PathNotAllowed(_) => StatusCode::FORBIDDEN,
            OacError::Database(_) => StatusCode::INTERNAL_SERVER_ERROR,
            OacError::CommandFailed(_) => StatusCode::INTERNAL_SERVER_ERROR,
            OacError::Validation(_) => StatusCode::BAD_REQUEST,
            OacError::Internal(_) => StatusCode::INTERNAL_SERVER_ERROR,
        };
        Self(status, e)
    }
}
