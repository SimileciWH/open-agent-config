...
use axum::Json;
use oac_core::models::{AgentDetail, AgentInfo, AgentCapabilities, ExtensionCounts, ExtensionKind, AgentConfigFile, ConfigCategory, ConfigScope};
...
pub async fn add_custom_config_path(
    ...
    blocking(move || {
        let resolved = oac_core::sanitize::resolve_and_validate_config_path(&params.path)?;
        let store = state.store.lock();
