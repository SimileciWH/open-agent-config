...
use axum::Json;
use oac_core::models::{Extension, ExtensionKind};
use oac_core::service::ExtensionContent;
use oac_core::{manager, scanner, service};
...
pub async fn scan_and_sync(
    ...
    let (count, unlinked) = tokio::task::spawn_blocking(move || {
        ...
        Ok::<_, oac_core::OacError>((count, unlinked))
    })
    .await
    .map_err(|e| ApiError::from(oac_core::OacError::Internal(e.to_string())))??;
            ...
            for name in &unique_names {
                if let Ok(results) = oac_core::marketplace::search_skills(name, 5) {
                    ...
                    if exact.len() == 1 {
                        ...
                        let remote_rev = oac_core::manager::get_remote_head(&git_url).ok();
            ...
            if !matched.is_empty() {
                ...
                for (id, name) in &unlinked {
                    if let Some((git_url, skill_id, remote_rev)) = matched.get(name.as_str()) {
                        let meta = oac_core::models::InstallMeta {
...
pub async fn list_skill_files(
    ...
    blocking(move || {
        ...
        if !path.exists() || !path.is_dir() {
            return Err(oac_core::OacError::NotFound("Directory not found".into()));
        }
        // Validate path is within an allowed agent directory
        let canonical = path.canonicalize()
            .map_err(|_| oac_core::OacError::NotFound("Cannot resolve path".into()))?;
        ...
        if !allowed {
            ...
            if !super::is_path_allowed(&canonical, &store) {
                return Err(oac_core::OacError::PathNotAllowed(
