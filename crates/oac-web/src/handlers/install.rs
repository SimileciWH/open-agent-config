...
use axum::Json;
use oac_core::models::*;
use oac_core::{deployer, manager, marketplace, sanitize, scanner, service};
    ...
    blocking(move || {
        let (target_dir, agent_name) = if let Some(ref agent) = params.target_agent {
            ...
            let a = state.adapters.iter().find(|a| a.detect())
                .ok_or_else(|| oac_core::OacError::Internal("No detected agent found".into()))?;
...
pub async fn install_from_marketplace(
    ...
    blocking(move || {
        ...
        let (target_dir, agent_name) = if let Some(ref agent) = params.target_agent {
            let a = state.adapters.iter()
                .find(|a| a.name() == agent.as_str())
                .ok_or_else(|| oac_core::OacError::Internal(format!("Agent '{}' not found", agent)))?;
            let dir = params
                .hermes_category
                .as_deref()
                .and_then(|cat| a.skill_dir_for_category(&params.target_scope, cat))
                .or_else(|| a.skill_dir_for(&params.target_scope))
                .ok_or_else(|| {
                    oac_core::OacError::Internal(format!(
                        "Agent '{}' has no skill directory for scope {:?}",
                        agent, params.target_scope
...
pub async fn install_from_local(
    ...
    blocking(move || {
        ...
        if !source_path.is_dir() {
            return Err(oac_core::OacError::Validation("Selected path is not a directory".into()));
        }
        ...
        for agent_name in &agents {
            let a = state.adapters.iter()
                ...
                .ok_or_else(|| oac_core::OacError::NotFound(format!("Agent '{}' not found", agent_name)))?;
    ...
    blocking(move || {
        ...
        let (ext_name, ext_kind) = {
            ...
            let ext = store.get_extension(&params.extension_id)?
                .ok_or_else(|| oac_core::OacError::NotFound("Extension not found".into()))?;
            (ext.name, ext.kind)
        };
    ...
    blocking(move || {
        let (ext, install_meta) = {
            ...
            let ext = store.get_extension(&params.id)?
                .ok_or_else(|| oac_core::OacError::NotFound(format!("Extension '{}' not found", params.id)))?;
            ...
            let meta = ext.install_meta.clone().ok_or_else(|| {
                oac_core::OacError::NotFound("Extension has no install metadata — cannot update".into())
            })?;
        ...
        // reflects it. Falls through to OAC's clone+deploy when delegation declines.
        ...
        let url = install_meta.url_resolved.as_deref()
            ...
            .ok_or_else(|| oac_core::OacError::NotFound("Extension has no remote URL".into()))?;

        // Clone the repo once
        let temp = tempfile::tempdir().map_err(|e| oac_core::OacError::Internal(e.to_string()))?;
        let clone_dir = temp.path().join("repo");
        let output = std::process::Command::new("git")
            .args(["clone", "--depth", "1", "--", url, &clone_dir.to_string_lossy()])
            .output()
            .map_err(|e| oac_core::OacError::CommandFailed(format!("Failed to run git clone: {}", e)))?;
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(oac_core::OacError::CommandFailed(format!("git clone failed: {}", stderr.trim())));
        }
        ...
        let skill_source = match manager::find_skill_in_repo(&clone_dir, skill_name) {
            Some(path) => path,
            None => {
                eprintln!("[oac] Skill '{}' no longer exists in repository — skipping update", skill_name);
                let store = state.store.lock();
                let now = chrono::Utc::now();
                if let Err(e) = store.update_check_state(&params.id, None, now, Some("removed_from_repo")) {
                    eprintln!("[oac] warning: {e}");
                }
                return Ok(manager::InstallResult {
                    name: skill_name.clone(),
                    was_update: false,
        ...
        for sibling in &all_siblings {
            let source_path = sibling.source_path.as_deref()
                .ok_or_else(|| oac_core::OacError::Internal("Sibling extension has no source_path".into()))?;
            let skill_dir = std::path::Path::new(source_path).parent().ok_or_else(|| {
                oac_core::OacError::Internal("Cannot determine skill directory from source path".into())
            })?;
            ...
            1 => {
                let agents = if params.target_agents.is_empty() {
                    vec![state.adapters.iter().find(|a| a.detect())
                        ...
                        .ok_or_else(|| oac_core::OacError::NotFound("No detected agent found".into()))?]
                } else {
                    params.target_agents
                };
                ...
                Ok(ScanResult::Installed {
                    result: last_result.ok_or_else(|| oac_core::OacError::Internal("No install results produced".into()))?,
                })
            }
            _ => {
...
pub async fn install_scanned_skills(
    ...
    let pending = {
        ...
        clones.remove(&params.clone_id)
            .ok_or_else(|| ApiError::from(oac_core::OacError::Internal("Clone session expired. Please try again.".into())))?
    };
...
pub async fn install_new_repo_skills(
    ...
    blocking(move || {
        let temp = tempfile::tempdir()
            .map_err(|e| oac_core::OacError::Internal(format!("Failed to create temp directory: {e}")))?;
        let clone_dir = temp.path().join("repo");
        let output = std::process::Command::new("git")
            .args(["clone", "--depth", "1", "--", &params.url, &clone_dir.to_string_lossy()])
            .output()
...
pub async fn get_cli_with_children(
    ...
    blocking(move || {
        ...
        let cli = store.get_extension(&params.cli_id)?
            .ok_or_else(|| oac_core::OacError::NotFound("CLI extension not found".into()))?;
    ...
    blocking(move || {
        ...
        let (updatable, unlinked): ExtensionUpdateBuckets = {
            ...
            if let Err(e) = service::migrate_pack_to_manual_meta(&store) {
                eprintln!("[oac] warning: pack→manual migration failed: {e}");
            }
            ...
            for (url, indices) in &url_to_indices {
                ...
                for &idx in indices {
                    ...
                    if needs_verify && manager::find_skill_in_repo(&clone_path, name).is_none() {
                        eprintln!("[oac] Skill '{}' no longer exists in repository", name);
