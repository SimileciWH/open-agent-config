...
use axum::Json;
use oac_core::models::DashboardStats;
use oac_core::{manager, service};
...
pub async fn get_dashboard_stats(
    ...
    blocking(move || {
        ...
        Ok(DashboardStats {
            ...
            skill_count: exts.iter().filter(|e| e.kind == oac_core::models::ExtensionKind::Skill).count(),
            mcp_count: exts.iter().filter(|e| e.kind == oac_core::models::ExtensionKind::Mcp).count(),
            plugin_count: exts.iter().filter(|e| e.kind == oac_core::models::ExtensionKind::Plugin).count(),
            hook_count: exts.iter().filter(|e| e.kind == oac_core::models::ExtensionKind::Hook).count(),
            cli_count: exts.iter().filter(|e| e.kind == oac_core::models::ExtensionKind::Cli).count(),
...
pub async fn read_config_file_preview(
    ...
    blocking(move || {
        ...
        if !file_path.exists() {
            return Err(oac_core::OacError::NotFound("File not found".into()));
        }
        let canonical = std::fs::canonicalize(file_path)
            .map_err(|_| oac_core::OacError::NotFound("File not found".into()))?;

        if file_path.is_dir() {
            return Ok(render_dir_tree(file_path));
        }

        // OS gates whether the user can read this file; OAC does not impose
        // its own home/project boundary on read-only previews of user-typed
        // config paths.
        let content = std::fs::read_to_string(&canonical).map_err(oac_core::OacError::from)?;
