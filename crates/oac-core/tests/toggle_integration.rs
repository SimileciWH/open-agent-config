...
use oac_core::models::*;
use oac_core::scanner::scan_skill_dir;
...
fn test_plugin_disable_enable_roundtrip_store_level() {
    ...
    let value = oac_core::deployer::read_plugin_config(&settings_path, "test-plugin@marketplace")
        .unwrap()
    ...
    store
        ...
        .unwrap();
    oac_core::deployer::remove_plugin_entry(&settings_path, "test-plugin@marketplace").unwrap();
    ...
    oac_core::deployer::restore_plugin_entry(&settings_path, plugin_key, restore_value).unwrap();
...
/// when OAC is managing the extension (disabled_config is set).
#[test]
fn test_sync_preserves_enabled_after_toggle() {
    ...
    store.insert_extension(&ext).unwrap();
    store.set_enabled("plugin-1", false).unwrap();
    // Must set disabled_config so UPSERT knows OAC manages this extension
    store
    ...
    let fetched = store.get_extension("plugin-1").unwrap().unwrap();
    assert!(!fetched.enabled, "OAC-managed disable should survive rescan");
}

/// External disable (e.g. user runs `/plugin disable` in Claude Code)
/// should be reflected in OAC after rescan.
#[test]
fn test_rescan_syncs_external_disable() {
    ...
    // No disabled_config → OAC is not managing this → should sync
    assert!(
...
/// OAC-managed disable must NOT be overwritten by rescan.
#[test]
fn test_rescan_preserves_oac_managed_disable() {
    let dir = TempDir::new().unwrap();
    let store = Store::open(&dir.path().join("test.db")).unwrap();
    ...
    // Scanner says enabled (stale) but OAC has disabled_config
    let ext_enabled = sample_plugin("plugin-1", "claude");
    store.sync_extensions(&[ext_enabled]).unwrap();

    assert!(
        !store.get_extension("plugin-1").unwrap().unwrap().enabled,
        "OAC-managed disable must survive rescan"
    );
}
...
fn test_dsh_mcp_native_toggle_roundtrip() {
    use oac_core::adapter::dsh::DshAdapter;
    use oac_core::adapter::AgentAdapter;
...
"#,
    ...
    let exts = oac_core::scanner::scan_mcp_servers(&adapter);
    assert_eq!(exts.len(), 1);
    store.sync_extensions(&exts).unwrap();
    ...
    oac_core::manager::toggle_extension_with_adapters(&store, &adapters, &ext_id, false).unwrap();
    ...
    oac_core::manager::toggle_extension_with_adapters(&store, &adapters, &ext_id, true).unwrap();
    let servers = DshAdapter::with_home(dir.path().to_path_buf()).read_mcp_servers();
    assert!(servers[0].enabled);
}
