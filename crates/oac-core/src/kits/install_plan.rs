...
use crate::kits::manifest::validate_manifest_paths;
...
pub fn compute_kit_install_plan(
    ...
    project_path: &str,
    agent_name: &str,
) -> Result<Vec<PlanItem>, OacError> {
    let adapter = adapters
        .iter()
        .find(|a| a.name() == agent_name)
        .ok_or_else(|| OacError::NotFound(format!("Agent '{agent_name}' not found")))?;
    ...
    for ext in manifest.extensions.iter() {
        ...
        crate::sanitize::validate_path_within(&project_root, &target)
