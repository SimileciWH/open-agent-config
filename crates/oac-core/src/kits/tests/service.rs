...
use crate::kits::manifest::{KIT_FORMAT_VERSION, KitManifest, ManifestExtension, sha256_of_bytes};
use crate::kits::service::{
    create_kit, delete_kit, export_kit, import_kit, list_kit_asset_candidates, list_kits,
    preview_kit_project_conflicts, sync_kit_to_project, unsync_kit_from_project, update_kit,
};
use crate::kits::types::{
    CreateKitRequest, KitConfigFileRef, PreviewKitConflictsRequest, SyncKitRequest,
    UnsyncKitRequest, UpdateKitRequest,
};
use crate::kits::zip_io::{PackEntry, pack_kit, read_manifest_from_zip};
...
fn create_kit_writes_zip_and_db_rows() {
    ...
    let store = Mutex::new(Store::open(&dir.path().join("oac.db")).unwrap());
    let adapters = all_adapters();

    let summary = make_basic_kit(
        &store,
        &adapters,
        dir.path(),
        "Empty-ext Test",
    ...
    let zip_path = dir
        .path()
        .join(".open-agent-config/kits")
        .join(format!("{}.oac-kit.zip", summary.id));
    assert!(
        zip_path.exists(),
        "zip should exist at {}",
        zip_path.display()
...
fn delete_kit_cascades_assets_and_stack_but_keeps_sync_records() {
    ...
    assert!(
        st.get_sync_record(&summary.id, "/tmp/p", "claude")
            .unwrap()
            .is_some()
    );
}
...
fn export_is_byte_identical_copy() {
    ...
    let target = dir.path().join("exported.oac-kit.zip");
...
fn import_generates_new_id_and_dedupes_name() {
    ...
    let exported = dir.path().join("shared.oac-kit.zip");
...
fn import_rejects_path_traversal_in_manifest() {
    ...
    let bad_zip = dir.path().join("bad.oac-kit.zip");
    {
...
#[test]
#[serial(kit_env)]
fn import_rejects_path_traversal_in_manifest_extension_name() {
    let dir = tempdir().unwrap();
    let _restore = RestoreHome::new();
    unsafe {
        std::env::set_var("HOME", dir.path());
    ...
    let bad_zip = dir.path().join("bad-extension-name.oac-kit.zip");
    let manifest = KitManifest {
        kit_format_version: KIT_FORMAT_VERSION,
        kit_id: "bad-extension-name".into(),
        name: "bad-extension-name".into(),
        description: "".into(),
        created_at: Utc::now(),
    ...
    pack_kit(
        ...
        &[PackEntry {
            ...
            bytes: b"# malicious".to_vec(),
        }],
    )
    .unwrap();

    let error = import_kit(&store, &bad_zip.to_string_lossy()).unwrap_err();
    assert!(
        matches!(error, crate::OacError::Validation(_)),
        "expected manifest name validation, got: {error}"
    );
}

#[test]
#[serial(kit_env)]
fn unsync_rejects_recorded_paths_outside_project() {
    let dir = tempdir().unwrap();
    let _restore = RestoreHome::new();
    unsafe {
        std::env::set_var("HOME", dir.path());
    ...
    let store = Mutex::new(Store::open(&dir.path().join("oac.db")).unwrap());
    let adapters = all_adapters();
    let kit = make_basic_kit(&store, &adapters, dir.path(), "Unsafe record", "x");
    ...
    store
        .lock()
        .upsert_sync_record(&crate::store::SyncRecordRow {
            id: uuid::Uuid::new_v4().to_string(),
            kit_id: kit.id.clone(),
            project_path: project.to_string_lossy().into(),
            agent_name: "claude".into(),
            written_paths: vec![outside.to_string_lossy().into()],
            synced_at: Utc::now(),
        })
        .unwrap();

    let error = unsync_kit_from_project(
        &store,
        &adapters,
        UnsyncKitRequest {
            kit_id: kit.id.clone(),
    ...
    .unwrap_err();
    assert!(matches!(error, crate::OacError::PathNotAllowed(_)));
    assert!(outside.join("sentinel").exists());
    assert!(
        store
            .lock()
            .get_sync_record(&kit.id, &project.to_string_lossy(), "claude")
            .unwrap()
            .is_some(),
        "failed unsync must preserve the record for later recovery"
    );
}

#[test]
#[serial(kit_env)]
fn unsync_rejects_existing_recorded_path_when_project_is_missing() {
    let dir = tempdir().unwrap();
    let _restore = RestoreHome::new();
    unsafe {
        std::env::set_var("HOME", dir.path());
    ...
    let store = Mutex::new(Store::open(&dir.path().join("oac.db")).unwrap());
    let adapters = all_adapters();
    let kit = make_basic_kit(&store, &adapters, dir.path(), "Missing project", "x");
    ...
    store
        ...
        .upsert_sync_record(&crate::store::SyncRecordRow {
            id: uuid::Uuid::new_v4().to_string(),
            kit_id: kit.id.clone(),
            project_path: missing_project.to_string_lossy().into(),
            agent_name: "claude".into(),
            written_paths: vec![outside.to_string_lossy().into()],
            synced_at: Utc::now(),
        })
    ...
    assert!(
        store
            .lock()
            .get_sync_record(&kit.id, &missing_project.to_string_lossy(), "claude")
            .unwrap()
            .is_some(),
        "failed unsync must preserve the record for later recovery"
    );
...
#[test]
#[serial(kit_env)]
fn legacy_kit_import_preserves_cross_machine_kind_counts() {
    // Simulates Bob receiving Alice's legacy .hk-kit.zip archive. The
    ...
    let manifest = KitManifest {
        ...
        description: "".into(),
        created_at: Utc::now(),
        exported_from: "legacy exporter test".into(),
        extensions: vec![
            ManifestExtension {
                name: "skill-a".into(),
                kind: ExtensionKind::Skill,
    ...
    assert_eq!(
        imported.extension_count, 2,
        "extension_count is fed by asset rows"
    );
    assert_eq!(
...
fn import_rejects_future_kit_format_version() {
    ...
    let zip_path = dir.path().join("future.oac-kit.zip");
    {
...
fn update_kit_changes_name_preserves_created_at() {
    ...
    assert_eq!(
        ...
        "created_at must be preserved across update_kit"
    );
    assert!(
        updated.updated_at > original_updated_at,
        "updated_at must bump on update_kit"
    );
}
...
fn make_mcp_kit(
    ...
    let zip_path = zip_dir.join(format!("{kit_id}.oac-kit.zip"));
    pack_kit(
        &zip_path,
        &manifest,
        &[PackEntry {
...
fn sync_mcp_merges_into_existing_mcp_json() {
    ...
    assert_eq!(
        ...
        "should have installed 1 extension"
    );
    assert_eq!(result.skipped_conflict_count, 0);
...
fn sync_mcp_writes_wrapped_format_on_empty_project() {
    ...
    let content: serde_json::Value = serde_json::from_str(&fs::read_to_string(&mcp_path).unwrap())
...
fn create_kit_rejects_hook_extension() {
    ...
    let now = chrono::Utc::now();
    let kit_id = uuid::Uuid::new_v4().to_string();
    let hook_zip = dir.path().join(format!("{kit_id}.oac-kit.zip"));
    ...
    assert!(
        msg.contains("Hook")
            && (msg.contains("v1") || msg.contains("not Kit-able") || msg.contains("redesign")),
...
fn make_store() -> (Store, tempfile::TempDir) {
    let dir = tempdir().unwrap();
    let store = Store::open(&dir.path().join("oac.db")).unwrap();
    (store, dir)
}
...
fn insert_kit_asset(store: &Mutex<Store>, kit_id: &str, ext_id: &str, kind: ExtensionKind) {
    let now = Utc::now();
    let ext = Extension {
        id: ext_id.into(),
        kind,
...
fn sync_skill_writes_install_meta_url_from_manifest() {
    ...
    let ext_id = crate::scanner::stable_id_with_scope_for("my-skill", "skill", "claude", &scope);
...
fn sync_skill_leaves_install_meta_null_when_manifest_url_is_none() {
    ...
    let zip_path = dir.path().join(format!("{kit_id}.oac-kit.zip"));
    pack_kit(
        &zip_path,
        &manifest,
        &[PackEntry {
    ...
    let ext_id = crate::scanner::stable_id_with_scope_for("local-skill", "skill", "claude", &scope);
    ...
    let ext_after = store.lock().get_extension(&ext_id).unwrap();
    assert!(
        ext_after.as_ref().is_none_or(|e| e.install_meta.is_none()),
