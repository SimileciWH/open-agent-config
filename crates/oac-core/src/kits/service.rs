...
use crate::kits::manifest::{
    KIT_FORMAT_VERSION, KitManifest, ManifestConfigFile, ManifestExtension, sha256_of_bytes,
    sha256_of_dir, validate_manifest_paths,
};
use crate::kits::paths::ensure_kits_dir;
use crate::kits::source_resolver::{
    ExtensionLocation, find_extension_source_by_id, strip_secrets_from_env,
};
use crate::kits::types::{
    ...
    KitSummary, KitSyncResult, KitSyncTarget, PreviewKitConflictsRequest, SyncKitRequest,
    UnsyncKitRequest, UpdateKitRequest,
};
use crate::kits::zip_io::{
    PackEntry, extract_entry_to_path, extract_prefix_to_dir, pack_kit, read_manifest_from_zip,
};
...
const EXPORTED_FROM_STRING: &str = concat!("Open Agent Config ", env!("CARGO_PKG_VERSION"));
...
const HOOK_V1_NOT_KITABLE: &str = "Hook extensions are not Kit-able (sync deferred)";

/// List all Kits as summaries (counts + corrupt flag).
pub fn list_kits(store: &Mutex<Store>) -> Result<Vec<KitSummary>, OacError> {
    ...
    let kind_by_ext_id: std::collections::HashMap<String, ExtensionKind> =
        all_exts.into_iter().map(|e| (e.id, e.kind)).collect();
...
pub fn create_kit(
    ...
    list_kits(store)?
        .into_iter()
        .find(|k| k.id == kit_id)
        .ok_or_else(|| OacError::Internal("created Kit not found in list".into()))
}
...
pub fn update_kit(
    ...
    let original = {
        ...
        store
            .get_kit_row(&req.id)?
            .ok_or_else(|| OacError::NotFound("Kit not found".into()))?
    };
    let now = Utc::now();
    pack_and_persist_kit(
        store,
    ...
    list_kits(store)?
        .into_iter()
        .find(|k| k.id == req.id)
        .ok_or_else(|| OacError::Internal("updated Kit not found in list".into()))
}

fn validate_kit_input(name: &str) -> Result<(), OacError> {
    if name.trim().is_empty() {
        return Err(OacError::Validation("Kit name must not be empty".into()));
    }
    Ok(())
}
...
fn check_kit_name_available(
    ...
    drop(st);
    if exists {
        return Err(OacError::Conflict(format!("kit-name-exists:{trimmed}")));
    }
    Ok(())
}
...
fn pack_and_persist_kit(
    ...
    let zip_path = zip_dir.join(format!("{kit_id}{}", crate::app_paths::KIT_SUFFIX));
...
fn resolve_and_pack_extensions(
    ...
    adapters: &[Box<dyn AgentAdapter>],
    extension_ids: &[String],
) -> Result<(Vec<ManifestExtension>, Vec<PackEntry>, Vec<String>), OacError> {
    ...
    for (position, id) in extension_ids.iter().enumerate() {
        let ext = extensions
            ...
            .find(|e| &e.id == id)
            .cloned()
            .ok_or_else(|| OacError::NotFound(format!("Extension '{id}' not found in store")))?;
        let location =
        ...
        let manifest_source_branch = ext.install_meta.as_ref().and_then(|m| m.branch.clone());
...
fn embed_extension(
    ...
    ext_name: &str,
    adapters: &[Box<dyn AgentAdapter>],
) -> Result<(String, String, Vec<PackEntry>, bool), OacError> {
    match kind {
        ExtensionKind::Skill | ExtensionKind::Cli => {
            ...
            if base.is_file() && matches!(kind, ExtensionKind::Skill) {
                let bytes = std::fs::read(base).map_err(|e| {
                    OacError::Internal(format!("pack standalone skill {}: {e}", base.display()))
                })?;
            ...
            .map_err(|e| OacError::Internal(format!("pack skill dir {}: {e}", base.display())))?;
            let hash = sha256_of_dir(base)
                .map_err(|e| OacError::Internal(format!("hash skill dir: {e}")))?;
            Ok((asset_prefix.to_string(), hash, entries, false))
        }
        ExtensionKind::Hook => Err(OacError::Validation(HOOK_V1_NOT_KITABLE.into())),
        ExtensionKind::Mcp => {
            ...
            let agent_name = loc.agent.as_deref().ok_or_else(|| {
                OacError::Internal("MCP source location missing adapter binding".into())
            })?;
            let adapter = adapter_for_agent(adapters, agent_name).ok_or_else(|| {
                OacError::NotFound(format!("Adapter '{agent_name}' not registered"))
            })?;
            ...
            let stripped =
                strip_secrets_from_env(&mut entry.env) | strip_secrets_from_env(&mut entry.headers);
            let bytes = serde_json::to_vec_pretty(&entry)
                .map_err(|e| OacError::Internal(format!("serialize mcp entry: {e}")))?;
        ...
        ExtensionKind::Plugin => Err(OacError::Internal("Plugin kind is not Kit-able".into())),
    }
}

fn resolve_and_pack_configs(
    config_files: &[KitConfigFileRef],
) -> Result<(Vec<ManifestConfigFile>, Vec<PackEntry>), OacError> {
...
pub fn list_kit_asset_candidates(
    store: &Mutex<Store>,
    adapters: &[Box<dyn AgentAdapter>],
) -> Result<KitAssetCandidates, OacError> {
    let store = store.lock();
    let all_exts = store.list_extensions(None, None)?;
...
pub fn get_kit_details(
    ...
    for asset in &assets {
        let manifest_entry = manifest.as_ref().and_then(|m| {
            m.extensions
                .iter()
                .find(|e| e.source_extension_id == asset.extension_id)
        });
        let kind = manifest_entry
            .map(|m| m.kind)
            .unwrap_or(ExtensionKind::Skill);
    ...
    let sync_targets = syncs
        .into_iter()
        .map(|s| KitSyncTarget {
            shared_with: shared_skill_dir_agents(adapters, &s.agent_name, &s.project_path),
...
) -> Result<KitConflictPreview, OacError> {
    let zip_path = {
        let store = store.lock();
        store
...
pub fn delete_kit(store: &Mutex<Store>, id: &str) -> Result<(), OacError> {
...
pub fn sync_kit_to_project(
    ...
    let project_root = std::path::Path::new(&req.project_path);
    ...
    for item in &plan {
        ...
        crate::sanitize::validate_path_within(project_root, &item.target_path)
            .map_err(|error| OacError::PathNotAllowed(error.to_string()))?;
    ...
    if !deployed_skill_metas.is_empty() {
        let adapter = adapter_for_agent(adapters, &req.agent_name)
            .ok_or_else(|| OacError::NotFound(format!("Agent '{}' not found", req.agent_name)))?;
        ...
        let scanned = scanner::scan_project_extensions(adapter, &project_name, &project_path_buf);
        let store_guard = store.lock();
        for ext in &scanned {
        ...
        for m in &deployed_skill_metas {
            let ext_id =
                scanner::stable_id_with_scope_for(&m.asset_name, "skill", &req.agent_name, &scope);
...
fn build_config_merge_groups(
    ...
    let mut groups: std::collections::HashMap<std::path::PathBuf, Vec<&install_plan::PlanItem>> =
        std::collections::HashMap::new();
    for item in plan {
    ...
    match kind {
        ...
        ExtensionKind::Mcp => {
            ...
            let mut entry_struct: McpServerEntry = serde_json::from_slice(&entry_bytes)
                .map_err(|e| OacError::ConfigCorrupted(format!("mcp entry json: {e}")))?;
            entry_struct.name = asset_name.clone();
            let adapter = adapter_for_agent(adapters, agent_name)
                .ok_or_else(|| OacError::NotFound(format!("Agent '{}' not found", agent_name)))?;
...
pub fn unsync_kit_from_project(
    ...
    let record = {
        ...
        store
            ...
            .ok_or_else(|| OacError::NotFound("Sync record not found".into()))?
    };
    let project_root = std::path::Path::new(&record.project_path);
    for entry in &record.written_paths {
        let path = if let Some(rest) = entry.strip_prefix("mcp:") {
            let Some((path, _name)) = rest.rsplit_once(':') else {
                continue;
            };
        ...
        if project_root.is_dir() {
            crate::sanitize::validate_path_within(project_root, path)
                .map_err(|error| OacError::PathNotAllowed(error.to_string()))?;
        } else if path.exists() {
            return Err(OacError::PathNotAllowed(format!(
                "cannot verify recorded path while project root is missing: {}",
                path.display()
            )));
        }
    }
    ...
    let manifest_skill_names: Vec<String> = {
        ...
        store
            ...
            .map(|m| {
                m.extensions
                    .into_iter()
                    .filter(|e| matches!(e.kind, ExtensionKind::Skill) && e.source_url.is_some())
                    .map(|e| e.name)
                    .collect()
            })
            .unwrap_or_default()
    ...
    if !manifest_skill_names.is_empty() {
        ...
        for skill_name in &manifest_skill_names {
            let ext_id =
                scanner::stable_id_with_scope_for(skill_name, "skill", &req.agent_name, &scope);
...
pub fn export_kit(store: &Mutex<Store>, kit_id: &str, target_path: &str) -> Result<(), OacError> {
    let row = {
        let store = store.lock();
        store
            .get_kit_row(kit_id)?
    ...
    if !std::path::Path::new(&row.zip_path).exists() {
        return Err(OacError::Internal("Kit zip is missing on disk".into()));
    }
    std::fs::copy(&row.zip_path, target_path)?;
    Ok(())
}

pub fn import_kit(store: &Mutex<Store>, source_zip_path: &str) -> Result<KitSummary, OacError> {
    let src = std::path::Path::new(source_zip_path);
    if !src.exists() {
        return Err(OacError::NotFound("Source zip not found".into()));
    }
    ...
    // Validate every entry path first to catch path traversal.
    {
        ...
        let mut a = zip::ZipArchive::new(f).map_err(|e| OacError::Internal(format!("zip: {e}")))?;
        for i in 0..a.len() {
            let entry = a
                .by_index(i)
                .map_err(|e| OacError::Internal(format!("zip: {e}")))?;
...
fn read_zip_entry_bytes(zip_path: &std::path::Path, entry_name: &str) -> Result<Vec<u8>, OacError> {
    let f = std::fs::File::open(zip_path)?;
    let mut archive =
        zip::ZipArchive::new(f).map_err(|e| OacError::Internal(format!("zip open: {e}")))?;
...
fn dedupe_name(desired: &str, existing: &[&str]) -> Result<String, OacError> {
    ...
    Err(OacError::Validation(format!(
