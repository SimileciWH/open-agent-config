...
pub fn deploy_skill(source_path: &Path, target_skill_dir: &Path) -> Result<String, OacError> {
    std::fs::create_dir_all(target_skill_dir)?;
    if source_path.is_dir() {
        let dir_name = source_path
            .file_name()
            .ok_or_else(|| OacError::Validation("Invalid source path".into()))?
            .to_string_lossy()
            .to_string();
...
fn copy_dir_recursive(src: &Path, dst: &Path) -> Result<(), OacError> {
...
pub fn deploy_mcp_server(
    ...
    match adapter.mcp_format() {
        ...
        McpFormat::DshCordis => Err(OacError::Validation(
...
fn validate_remote_mcp_target(
    ...
    if entry.url.is_none() {
        return Err(OacError::ConfigCorrupted(format!(
            "Remote MCP server '{}' has no url",
            entry.name
        )));
    }
    match schema {
        RemoteMcpSchema::Unsupported => Err(OacError::Validation(format!(
            "{agent_name} does not support remote (HTTP/SSE) MCP servers"
        ))),
        RemoteMcpSchema::Toml if entry.transport == McpTransport::Sse => Err(OacError::Validation(
...
) -> Result<serde_json::Value, OacError> {
    if entry.transport == McpTransport::Stdio {
...
fn deploy_mcp_server_json(
    ...
    locked_modify_json(config_path, |config| {
        let servers = config
            .as_object_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("Config is not an object".into()))?
            .entry(top_key)
            .or_insert_with(|| serde_json::json!({}));
        servers
            .as_object_mut()
            .ok_or_else(|| OacError::ConfigCorrupted(format!("{} is not an object", top_key)))?
...
fn deploy_mcp_server_toml(config_path: &Path, entry: &McpServerEntry) -> Result<(), OacError> {
...
/// The original name is stored as `_oac_name` so the scanner can recover it
...
fn upsert_mcp_server_toml(
    ...
    let parent = config_path
        .parent()
        .ok_or_else(|| OacError::Validation("Invalid config path".into()))?;
    ...
    let mut doc: toml::Table = if existing.is_empty() {
        ...
        existing
            .parse::<toml::Table>()
            .map_err(|e| OacError::ConfigCorrupted(format!("Failed to parse TOML config: {e}")))?
    };
    ...
    let mcp_servers = doc
        ...
        .as_table_mut()
        .ok_or_else(|| OacError::ConfigCorrupted("mcp_servers is not a table".into()))?;

    let safe_name = sanitize_mcp_name(name);
    if safe_name != name {
        server_val
            .as_table_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("MCP server entry is not a table".into()))?
            .insert("_oac_name".into(), toml::Value::String(name.to_string()));
    }
    ...
    atomic_write(
        config_path,
        &toml::to_string_pretty(&doc).map_err(|e| OacError::Internal(e.to_string()))?,
    )?;
...
///     `"remote"` variant that Open Agent Config does not deploy)
///
...
fn deploy_mcp_server_opencode(config_path: &Path, entry: &McpServerEntry) -> Result<(), OacError> {
...
fn modify_hermes_yaml(
    config_path: &Path,
    f: impl FnOnce(&mut serde_yaml::Mapping) -> Result<(), OacError>,
) -> Result<(), OacError> {
    ...
    let mut doc: serde_yaml::Value = if existing.trim().is_empty() {
        ...
        serde_yaml::from_str(&existing).map_err(|e| {
            OacError::ConfigCorrupted(format!("Failed to parse Hermes config.yaml: {e}"))
        })?
    };
    let root = doc
        .as_mapping_mut()
        .ok_or_else(|| OacError::ConfigCorrupted("config.yaml root is not a mapping".into()))?;
    f(root)?;
    let output = serde_yaml::to_string(&doc).map_err(|e| OacError::Internal(e.to_string()))?;
...
fn deploy_mcp_server_hermes_yaml(
    ...
    modify_hermes_yaml(config_path, |root| {
        let servers = root
            ...
            .as_mapping_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("mcp_servers is not a mapping".into()))?;
...
pub fn set_hermes_plugin_enabled(
    ...
    modify_hermes_yaml(config_path, |root| {
        let plugins = root
            ...
            .or_insert_with(|| serde_yaml::Value::Mapping(serde_yaml::Mapping::new()))
            .as_mapping_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("plugins is not a mapping".into()))?;
        let list = plugins
            .entry("enabled".into())
            .or_insert_with(|| serde_yaml::Value::Sequence(vec![]))
            .as_sequence_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("plugins.enabled is not a sequence".into()))?;
...
/// `enabled` toggles — unlike Open Agent Config's generic MCP disable, it never removes
...
pub fn set_hermes_mcp_enabled(
    ...
    modify_hermes_yaml(config_path, |root| {
        ...
        let server = servers
            ...
            .and_then(|v| v.as_mapping_mut())
            .ok_or_else(|| {
                OacError::NotFound(format!("MCP server '{name}' not found in config"))
            })?;
...
pub fn set_kiro_mcp_enabled(
    ...
    locked_modify_json(config_path, |config| {
        let servers = config
            ...
            .ok_or_else(|| OacError::NotFound("No mcpServers block found".into()))?;
        let server = servers
            .get_mut(server_name)
            .and_then(|v| v.as_object_mut())
            .ok_or_else(|| OacError::NotFound(format!("MCP server '{server_name}' not found")))?;
        if enabled {
...
const DSH_BLOCK_BEGIN: &str = "# >>> managed by Open Agent Config — do not edit this block >>>";
const DSH_BLOCK_END: &str = "# <<< managed by Open Agent Config <<<";
const LEGACY_DSH_BLOCK_BEGIN: &str = "# >>> managed by HarnessKit — do not edit this block >>>";
const LEGACY_DSH_BLOCK_END: &str = "# <<< managed by HarnessKit <<<";

/// Flip a dsh MCP server via the official patch-layer mechanism: an
/// id-targeted `disabled:` override inside an OAC-owned marked block at the
...
/// the block is OAC-owned by contract. Both marker generations are accepted,
/// while every rewrite emits only the current OAC markers. `split_inclusive` keeps user lines
...
fn split_dsh_managed_block(
    text: &str,
) -> Result<(String, std::collections::BTreeMap<String, bool>), OacError> {
    ...
    for raw in text.split_inclusive('\n') {
        let line = raw.trim();
        let begin_end = match line {
            DSH_BLOCK_BEGIN => Some(DSH_BLOCK_END),
            LEGACY_DSH_BLOCK_BEGIN => Some(LEGACY_DSH_BLOCK_END),
            _ => None,
        };
        if let Some(end_marker) = begin_end {
            if expected_end.is_some() {
                return Err(OacError::ConfigCorrupted(
                    "unbalanced OAC managed-block markers: \
                     nested BEGIN marker inside the managed block"
                        .into(),
                ));
            }
            expected_end = Some(end_marker);
            continue;
        }
        if line == DSH_BLOCK_END || line == LEGACY_DSH_BLOCK_END {
            if expected_end != Some(line) {
                return Err(OacError::ConfigCorrupted(
                    "unbalanced OAC managed-block markers: \
                     END marker without its matching BEGIN"
                        .into(),
                ));
            }
            expected_end = None;
        ...
        if expected_end.is_some() {
            if let Some(id) = line.strip_prefix("- id: ") {
                ...
                && let (Some(id), Ok(b)) = (current_id.take(), flag.trim().parse::<bool>())
            {
                managed.insert(id, b);
            }
        } else {
...
pub fn set_kiro_hook_enabled(
    ...
    locked_modify_json(config_path, |config| {
        let hooks = config
            ...
            .ok_or_else(|| OacError::NotFound("No hooks array found".into()))?;
        let hook = hooks
            .iter_mut()
            .find(|h| kiro_hook_matches(h, event, matcher, command))
            .ok_or_else(|| OacError::NotFound(format!("Hook for '{event}' not found in config")))?;
        let obj = hook
            .as_object_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("hook is not an object".into()))?;
...
fn deploy_hook_hermes_yaml(config_path: &Path, entry: &HookEntry) -> Result<(), OacError> {
    modify_hermes_yaml(config_path, |root| {
        let hooks = root
            .entry("hooks".into())
            .or_insert_with(|| serde_yaml::Value::Mapping(serde_yaml::Mapping::new()))
            .as_mapping_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("hooks is not a mapping".into()))?;
        let list = hooks
            .entry(entry.event.clone().into())
            .or_insert_with(|| serde_yaml::Value::Sequence(vec![]))
            .as_sequence_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("hook event is not a sequence".into()))?;
        if list
            .iter()
...
fn restore_hook_hermes_yaml(
    ...
    let yaml_item: serde_yaml::Value =
        serde_yaml::to_value(entry).map_err(|e| OacError::Internal(e.to_string()))?;
    modify_hermes_yaml(config_path, |root| {
        let hooks = root
            .entry("hooks".into())
            .or_insert_with(|| serde_yaml::Value::Mapping(serde_yaml::Mapping::new()))
...
fn read_hook_config_hermes_yaml(
    ...
    matcher: Option<&str>,
    command: &str,
) -> Result<Option<serde_json::Value>, OacError> {
    if !config_path.exists() {
        return Ok(None);
    }
    ...
    let json_str = serde_json::to_string(item).map_err(|e| OacError::Internal(e.to_string()))?;
    let json_val =
        serde_json::from_str(&json_str).map_err(|e| OacError::Internal(e.to_string()))?;
    Ok(Some(json_val))
}
    ...
    locked_modify_json(config_path, |config| {
        match format {
            HookFormat::ClaudeLike => {
                ...
                let event_arr = hooks
                    .as_object_mut()
                    .ok_or_else(|| OacError::ConfigCorrupted("hooks is not an object".into()))?
                    .entry(&entry.event)
                    .or_insert_with(|| serde_json::json!([]));
                let arr = event_arr.as_array_mut().ok_or_else(|| {
                    OacError::ConfigCorrupted("hook event is not an array".into())
                })?;
            ...
            HookFormat::Cursor => {
                ...
                let arr = event_arr
                    .as_array_mut()
                    .ok_or_else(|| OacError::ConfigCorrupted("event is not an array".into()))?;
            ...
            HookFormat::KiroIde => {
                ...
                let arr = hooks
                    .as_array_mut()
                    .ok_or_else(|| OacError::ConfigCorrupted("hooks is not an array".into()))?;
                if !arr.iter().any(|h| {
            ...
            HookFormat::None => {
                return Err(OacError::Internal("Agent does not support hooks".into()));
            }
        }
        Ok(())
    })
    ...
    match format {
        McpFormat::Toml => {
            ...
            let mut doc: toml::Table = content
                .parse::<toml::Table>()
                .map_err(|e| OacError::ConfigCorrupted(e.to_string()))?;
...
/// any leading user-comments next to the removed entry stay in place — OAC
/// never edits user comment text, only its own data entries.
fn remove_mcp_server_opencode(config_path: &Path, server_name: &str) -> Result<(), OacError> {
    locked_modify_jsonc(config_path, |root| {
...
pub fn remove_plugin_entry(config_path: &Path, plugin_key: &str) -> Result<(), OacError> {
    if !config_path.exists() {
        return Ok(());
    }
    locked_modify_json(config_path, |config| {
    ...
    match format {
        McpFormat::Toml => {
            ...
            let toml_val: toml::Value = serde_json::from_value(entry.clone())
                .map_err(|e| OacError::ConfigCorrupted(format!("saved MCP snapshot: {e}")))?;
        ...
        _ => {
            ...
            locked_modify_json(config_path, |config| {
                ...
                servers
                    .as_object_mut()
                    .ok_or_else(|| OacError::ConfigCorrupted(format!("{key} is not an object")))?
...
pub fn set_plugin_enabled(
    ...
    locked_modify_json(config_path, |config| {
        ...
        plugins
            .as_object_mut()
            .ok_or_else(|| OacError::ConfigCorrupted("enabledPlugins is not an object".into()))?
...
pub fn set_codex_plugin_enabled(
    ...
    let plugins = doc
        ...
        .or_insert_with(|| toml::Value::Table(toml::Table::new()))
        .as_table_mut()
        .ok_or_else(|| OacError::ConfigCorrupted("plugins is not a table".into()))?;
    let entry = plugins
        .entry(plugin_key)
        .or_insert_with(|| toml::Value::Table(toml::Table::new()))
        .as_table_mut()
        .ok_or_else(|| OacError::ConfigCorrupted("plugin entry is not a table".into()))?;
    entry.insert("enabled".into(), toml::Value::Boolean(enabled));

    let output = toml::to_string_pretty(&doc).map_err(|e| OacError::Internal(e.to_string()))?;
...
pub fn remove_codex_plugin_entry(config_path: &Path, plugin_key: &str) -> Result<(), OacError> {
    if !config_path.exists() {
        return Ok(());
    }
    let file = std::fs::OpenOptions::new()
...
pub fn set_vscode_plugin_enabled(
    ...
    let conn = rusqlite::Connection::open(&db_path)
        .map_err(|e| OacError::Internal(format!("Failed to open VS Code state DB: {}", e)))?;
    ...
    let new_value =
        serde_json::to_string(&entries).map_err(|e| OacError::Internal(e.to_string()))?;
    ...
    .map_err(|e| OacError::Internal(format!("Failed to update VS Code state DB: {}", e)))?;
...
pub fn set_gemini_extension_enabled(
    ...
    modify_gemini_enablement(extensions_dir, |config| {
        ...
        let overrides = entry
            .get_mut("overrides")
            .and_then(|v| v.as_array_mut())
            .ok_or_else(|| OacError::ConfigCorrupted("overrides is not an array".into()))?;
...
fn modify_gemini_enablement(
    extensions_dir: &Path,
    modify: impl FnOnce(&mut serde_json::Map<String, serde_json::Value>) -> Result<(), OacError>,
) -> Result<(), OacError> {
    let enablement_path = extensions_dir.join("extension-enablement.json");
    ...
    let mut config: serde_json::Map<String, serde_json::Value> = if content.is_empty() {
        ...
        serde_json::from_str(&content)
            .map_err(|e| OacError::ConfigCorrupted(format!("extension-enablement.json: {}", e)))?
    };

    modify(&mut config)?;

    let output =
        serde_json::to_string_pretty(&config).map_err(|e| OacError::Internal(e.to_string()))?;
    (&file).seek(SeekFrom::Start(0))?;
    file.set_len(0)?;
    (&file).write_all(output.as_bytes())?;
    (&file).flush()?;
...
pub fn read_mcp_server_config(
    ...
    match format {
        McpFormat::Toml => {
            ...
            // The scanner uses `_oac_name` to recover the original name, so
...
fn read_mcp_server_config_opencode(
    ...
    let cst = CstRootNode::parse(&content, &Default::default())
        .map_err(|e| OacError::ConfigCorrupted(format!("Failed to parse jsonc: {e}")))?;
...
fn read_or_create_json(path: &Path) -> Result<serde_json::Value, OacError> {
    if path.exists() {
...
#[allow(dead_code)]
fn write_json(path: &Path, value: &serde_json::Value) -> Result<(), OacError> {
...
fn atomic_write(path: &Path, content: &str) -> Result<(), OacError> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
...
fn locked_modify_jsonc<F>(path: &Path, modify: F) -> Result<(), OacError>
where
    F: FnOnce(&jsonc_parser::cst::CstObject) -> Result<(), OacError>,
{
    use jsonc_parser::cst::CstRootNode;
    ...
    let root_obj = cst
        .object_value()
        .ok_or_else(|| OacError::ConfigCorrupted("Config root is not an object".into()))?;
...
fn locked_modify_json<F>(path: &Path, modify: F) -> Result<(), OacError>
where
    F: FnOnce(&mut serde_json::Value) -> Result<(), OacError>,
{
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)?;
    }
...
mod tests {
    ...
    fn build_mcp_json_value_spells_each_remote_schema() {
        ...
        stdio.extra.insert(
            "cwd".into(),
            serde_json::Value::String("/tmp/project".into()),
        );
    ...
    fn validate_remote_mcp_target_rejects_unsupported_combinations() {
        ...
        let err = validate_remote_mcp_target(&http, "someagent", RemoteMcpSchema::Unsupported)
            .unwrap_err();
        assert!(matches!(&err, OacError::Validation(m) if m.contains("someagent")));

        // Codex (TOML) is HTTP-only.
        let err = validate_remote_mcp_target(&sse, "codex", RemoteMcpSchema::Toml).unwrap_err();
        assert!(matches!(&err, OacError::Validation(m) if m.contains("not SSE")));
        ...
        assert!(matches!(err, OacError::ConfigCorrupted(_)));

        let mut kimi_fields = remote_entry(McpTransport::Stdio);
        kimi_fields.extra.insert(
            "cwd".into(),
            serde_json::Value::String("/tmp/project".into()),
        );
        ...
        .unwrap_err();
        assert!(matches!(err, OacError::Validation(message) if message.contains("Kimi-specific")));
    }
    ...
    fn locked_modify_jsonc_rejects_non_object_root() {
        ...
        assert!(matches!(err, Err(OacError::ConfigCorrupted(_))));
        // File untouched.
    ...
    fn test_remove_mcp_server_opencode_preserves_comments() {
        ...
        // design — OAC never edits user comment text.
        let dir = TempDir::new().unwrap();
    ...
    fn test_restore_mcp_server_opencode_preserves_comments() {
        ...
        // the OAC toggle flow (disable → restore).
        let dir = TempDir::new().unwrap();
        let config = dir.path().join("opencode.jsonc");
        std::fs::write(
            &config,
    ...
    fn test_deploy_mcp_server_toml_sanitizes_name_and_preserves_original() {
        ...
        assert!(!servers.contains_key("microsoft/markitdown"));
        // Original name preserved in _oac_name for scanner round-trip
        let server = servers["microsoft-markitdown"].as_table().unwrap();
        assert_eq!(
            server["_oac_name"].as_str().unwrap(),
            "microsoft/markitdown"
        );
    ...
    #[test]
    fn test_deploy_mcp_server_toml_no_oac_name_when_unchanged() {
        let dir = TempDir::new().unwrap();
        let config = dir.path().join("config.toml");
        let entry = McpServerEntry {
            name: "context7".into(),
        ...
        let doc: toml::Table = std::fs::read_to_string(&config).unwrap().parse().unwrap();
        let server = doc["mcp_servers"]["context7"].as_table().unwrap();
        // No _oac_name needed when name didn't require sanitization.
        assert!(!server.contains_key("_oac_name"));
    }
    ...
    fn test_set_kiro_hook_enabled_flips_in_place() {
        ...
        assert!(matches!(err, OacError::NotFound(_)));
    }
...
"#;
    ...
    fn disable_appends_managed_block_and_enable_removes_it() {
        ...
        assert!(text.contains("managed by Open Agent Config"));
    ...
    #[test]
    fn legacy_managed_block_is_read_and_rewritten_with_oac_markers() {
        let text = format!(
            "{HOME_WITH_GH}{LEGACY_DSH_BLOCK_BEGIN}\n- id: mcp-github\n  disabled: true\n{LEGACY_DSH_BLOCK_END}\n"
        );
        ...
        assert!(out.contains(DSH_BLOCK_BEGIN));
        assert!(out.contains(DSH_BLOCK_END));
        assert!(!out.contains(LEGACY_DSH_BLOCK_BEGIN));
        assert!(!out.contains(LEGACY_DSH_BLOCK_END));
    }

    #[test]
    fn enable_overrides_user_disable_with_disabled_false() {
        // User disabled it themselves → OAC writes an explicit disabled: false
    ...
    fn user_content_after_block_survives_roundtrip() {
        ...
        assert!(
            !out.contains("managed by Open Agent Config"),
    ...
    fn remove_mcp_server_refuses_dsh_cordis() {
        ...
        assert!(matches!(&err, OacError::Validation(m) if m.contains("cordis.patch.yml")));
        assert_eq!(std::fs::read_to_string(&path).unwrap(), HOME_WITH_GH);
    }
}
