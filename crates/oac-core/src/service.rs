...
/// Whether an extension is eligible for OAC's update flow.
///
...
/// skill that just happens to share a name. Project skills installed by OAC
...
pub fn try_delegate_skill_update(store: &Mutex<Store>, ext: &Extension) -> Result<bool, OacError> {
    ...
    // `skills update` synced the skill to its upstream HEAD. OAC's update check is
        ...
        .or_else(|| ext.source.url.clone())
    {
        match manager::get_remote_head(&url) {
            ...
            Err(e) => eprintln!("[oac] delegated update: could not record revision: {e}"),
        }
    }
    Ok(true)
}
...
pub fn bind_pack(store: &Store, ids: &[String], pack: Option<&str>) -> Result<(), OacError> {
...
pub fn migrate_pack_to_manual_meta(store: &Store) -> Result<usize, OacError> {
...
pub fn run_full_audit(
    store: &Store,
    adapters: &[Box<dyn AgentAdapter>],
) -> Result<Vec<AuditResult>, OacError> {
    let extensions = store.list_extensions(None, None)?;
...
fn remove_path(path: &std::path::Path) -> Result<(), OacError> {
    if path.is_dir() {
...
// --- Extension command flows shared by oac-desktop and oac-web -------------
...
pub fn delete_extension(
    ...
    match ext.kind {
        ExtensionKind::Skill => {
            // Deleted natively even for skills-CLI-managed skills: OAC's delete is
...
pub fn get_extension_content(
    ...
    let (ext, projects) = {
        ...
        let ext = store
            .get_extension(id)?
            .ok_or_else(|| OacError::NotFound("Extension not found".into()))?;
        let projects = store.list_project_tuples();
        (ext, projects)
    };
    ...
    match ext.kind {
        ExtensionKind::Skill => {
            if let Some(loc) = scanner::find_skill_by_id(adapters, id, &ext.agents, &projects) {
                ...
                Err(OacError::NotFound("Skill file not found".into()))
            }
        }
        ExtensionKind::Mcp => {
...
pub fn install_to_agent(
    ...
    // A project-scope deploy must target a project OAC already tracks:
        ...
        && !projects.iter().any(|(_, p)| p == path)
    {
        return Err(OacError::Validation(format!(
            "Project '{path}' is not registered in Open Agent Config; add the project first"
        )));
    }
    ...
    let target_adapter = adapters
        ...
        .ok_or_else(|| OacError::NotFound(format!("Agent '{}' not found", target_agent)))?;
    ...
    match ext.kind {
        ExtensionKind::Skill => {
            let source_path =
                scanner::find_skill_by_id(adapters, extension_id, &ext.agents, &projects)
                    ...
                    .ok_or_else(|| OacError::Internal("Could not find source skill files".into()))?;
        ...
        ExtensionKind::Mcp => {
            ...
            let mut entry = source_entry.ok_or_else(|| {
                OacError::Internal("Could not find source MCP server config".into())
            })?;
        ...
        ExtensionKind::Hook => {
            ...
            let mut entry = source_entry
                .ok_or_else(|| OacError::Internal("Could not find source hook config".into()))?;
        ...
        ExtensionKind::Cli => {
            ...
            let source_path = locations
                ...
                .map(|(_, path)| path)
                .ok_or_else(|| {
                    OacError::Internal("Could not find source skill files for CLI".into())
                })?;
        ...
        other => Err(OacError::Internal(format!(
...
mod tests {
    ...
    fn test_is_update_eligible_project_skill() {
        ...
        // Project skill, has install_meta — eligible (OAC-installed).
    ...
    fn test_install_to_agent_rejects_unregistered_project() {
        ...
        assert!(
            matches!(&err, OacError::Validation(msg) if msg.contains("not registered")),
    ...
    fn test_install_to_agent_hermes_has_no_project_skills() {
        ...
        assert!(
            matches!(&err, OacError::Validation(msg) if msg.contains("no skill directory")),
    ...
    fn test_install_to_agent_remote_mcp_claude_to_codex() {
        ...
        .unwrap_err();
        assert!(
            matches!(&err, OacError::Validation(msg) if msg.contains("not SSE")),
    ...
    fn test_install_to_agent_mcp_project_scope_and_windsurf_rejection() {
        ...
        assert!(
            matches!(&err, OacError::Validation(msg) if msg.contains("project-level MCP")),
    ...
    fn test_install_to_agent_kiro_hook_project_ok_global_rejected() {
        ...
        .unwrap_err();
        assert!(
            matches!(&err, OacError::Validation(msg) if msg.contains("per-project")),
