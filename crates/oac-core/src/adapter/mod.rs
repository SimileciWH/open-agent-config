...
/// `enabled` is Open Agent Config-internal and defaults to `true` on the install
...
/// such an entry — OAC is a scanner, and showing the URL beats showing an
...
pub enum HookFormat {
    ...
    /// Only command actions map to Open Agent Config's shell-command HookEntry model.
    KiroIde,
...
pub trait AgentAdapter: Send + Sync {
    ...
    /// True if Open Agent Config should resolve bare commands to absolute paths and
    ...
    /// `enabled: bool` that Open Agent Config should toggle IN PLACE, instead of the
    ...
    /// when this is `false`, so OAC never writes a config the agent silently
    ...
    fn mcp_remote_capability_derivation() {
        ...
        for a in all_adapters() {
            ...
            match a.name() {
                ...
                "dsh" => {
                    // Remote schema Unsupported: OAC never deploys remote
