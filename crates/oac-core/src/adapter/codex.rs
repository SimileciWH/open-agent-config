...
impl CodexAdapter {
    ...
    /// names (all empty/non-string entries). OAC's role here is discovery, not
...
impl AgentAdapter for CodexAdapter {
    ...
    fn global_memory_files(&self) -> Vec<PathBuf> {
        ...
        // OCR'd screen content. Surfacing those to OAC's Memory section would
    ...
    fn project_hook_config_relpath(&self) -> Option<String> {
        ...
        // — OAC does not surface that form (asymmetric read/write would let
        // users see hooks they can't redeploy). Source:
        // https://developers.openai.com/codex/hooks
        //
        // Codex requires a per-project trust grant before these hooks
        // execute; the file exists on disk regardless, so OAC scans it as
    ...
    fn read_mcp_servers_from(&self, path: &Path) -> Vec<McpServerEntry> {
        ...
        servers
            ...
            .map(|(name, val)| {
                ...
                // Prefer `_oac_name` (original name before TOML key sanitization).
                // `_hk_name` is accepted only as a legacy migration input.
                // so that the scanner produces the same Extension name across all
                // agents. Falls back to the TOML key if neither marker exists.
                let canonical_name = table
                    .and_then(|t| t.get("_oac_name").or_else(|| t.get("_hk_name")))
                    .and_then(|v| v.as_str())
                    .map(String::from)
...
"#,
    ...
    #[test]
    fn read_mcp_servers_accepts_legacy_name_marker() {
        let tmp = tempfile::tempdir().unwrap();
        let config = tmp.path().join("config.toml");
        fs::write(
            &config,
...
"#,
        ...
        assert_eq!(servers[0].name, "legacy/name");
    }
...
"#,
    ...
    fn doc_fallback_treats_empty_array_as_no_override() {
        // OAC is a discovery tool: an explicit empty list would suppress all
