...
// ~/.omp/profiles/<name>/agent/ — OAC scans the default profile only.
//
...
impl OmpAdapter {
    ...
    fn plugin_name(path: &Path) -> String {
        ...
        // must be stable across enable/disable or OAC's toggle model breaks.
    ...
    ///
    /// A `.disabled` suffix is OAC's own in-place disable rename (same model as
    /// opencode plugins). omp's native disable mechanism is the
    /// `disabledExtensions` id list in config.yml, which OAC doesn't write; the
        ...
        for plugin_dir in self.plugin_dirs() {
            ...
            for file in files.flatten() {
                ...
                if path.is_file() {
                    ...
                    // path is the index file — OAC's toggle renames that file,
...
mod tests {
    ...
    fn read_plugins_lists_ts_extensions() {
        ...
        // at the index file so OAC's rename toggle keeps the name stable.
