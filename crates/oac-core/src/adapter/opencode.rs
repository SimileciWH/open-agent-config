...
impl OpencodeAdapter {
    ...
    fn parse_json(path: &Path) -> Option<serde_json::Value> {
        ...
        // opencode.jsonc — would silently empty OAC's MCP list under
    ...
    fn parse_mcp_entry(name: &str, value: &serde_json::Value) -> Option<McpServerEntry> {
        ...
        // matching how Open Agent Config displays user-disabled MCPs from every other
...
impl AgentAdapter for OpencodeAdapter {
    ...
    fn detect(&self) -> bool {
        ...
        // agent that has nothing for Open Agent Config to manage — a UX false
    ...
    fn mcp_config_path(&self) -> PathBuf {
        ...
        // conflict, so OAC's read picks the file whose values are actually in
        // effect, and OAC's writes target the file the user is using. Falls
...
mod tests {
    ...
    fn read_mcp_servers_surfaces_disabled_state() {
        ...
        // via `enabled: false`. Open Agent Config surfaces these entries as visible
        ...
        // what Open Agent Config displayed. Default-true (omitted or explicit) flows
        // through as enabled.
        let tmp = tempfile::tempdir().unwrap();
        let config_dir = tmp.path().join(".config/opencode");
        std::fs::create_dir_all(&config_dir).unwrap();
    ...
    fn read_mcp_servers_accepts_jsonc_comments_in_dot_json() {
        ...
        // ConfigParse.jsonc). The most common OAC breakage isn't a .jsonc
        // file at all — it's a user adding a `//` comment or trailing comma
        // to opencode.json. With serde_json, OAC silently empties the MCP
    ...
    fn read_mcp_servers_picks_jsonc_when_present() {
        ...
        // opencode.json path. With smart selection in mcp_config_path(), OAC
    ...
    fn mcp_config_path_prefers_jsonc_when_both_exist() {
        ...
        // .json on conflict. OAC reads (and writes) the file whose values are
