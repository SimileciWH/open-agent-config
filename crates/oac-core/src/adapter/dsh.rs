...
//   (profile patch, then home patch); OAC reads the HOME layer only —
...
struct CordisRow {
    ...
    /// None — upstream evaluates js `disabled` at runtime; OAC can't, so it
...
fn parse_patch_rows(text: &str, origin: &Path) -> Vec<CordisRow> {
    let doc: serde_yaml::Value = match serde_yaml::from_str(text) {
        Ok(doc) => doc,
        Err(err) => {
            eprintln!("[oac] warning: cannot parse {}: {err}", origin.display());
            return vec![];
        }
    };
    let Some(items) = doc.as_sequence() else {
        eprintln!(
            "[oac] warning: {} is not a YAML list (cordis patch files are top-level arrays)",
            origin.display()
        );
        return vec![];
    };
...
fn yaml_disabled(v: Option<&serde_yaml::Value>) -> Option<bool> {
    match v {
        ...
        _ => None, // absent, or !!js expression (arrives as String — OAC can't evaluate)
    }
}
...
impl DshAdapter {
    ...
    /// to compute base state with OAC's managed block stripped).
...
impl AgentAdapter for DshAdapter {
    ...
    /// canonical write target for OAC's managed toggle block. NEVER point this
...
mod tests {
    ...
    fn config_discovery_paths() {
        ...
        // OAC-side project-discovery marker (dsh's only project-level dir).
    ...
    /// tells users to merge exactly this into the patch files OAC reads.
...
"#;
    ...
    fn mcp_enabled_map_reflects_folded_state() {
        ...
        assert!(map["github"], "disabled: null re-enables");
    }
