...
pub fn search_skills(query: &str, limit: usize) -> Result<Vec<MarketplaceItem>, OacError> {
    if query.len() < 2 {
        return Ok(vec![]);
    }
    let resp: SkillsShSearchResponse = client()
        ...
        .send()
        .map_err(|e| OacError::Network(format!("Failed to reach skills.sh: {e}")))?
        .error_for_status()
        .map_err(|e| OacError::Network(format!("skills.sh returned error: {e}")))?
        .json()
        .map_err(|e| OacError::Network(format!("Failed to parse skills.sh response: {e}")))?;
    Ok(resp
        .skills
        .into_iter()
...
pub async fn search_skills_async(
    query: &str,
    limit: usize,
) -> Result<Vec<MarketplaceItem>, OacError> {
    if query.len() < 2 {
        return Ok(vec![]);
    }
...
pub fn search_servers(query: &str, limit: usize) -> Result<Vec<MarketplaceItem>, OacError> {
    if query.len() < 2 {
        return Ok(vec![]);
    }
    let resp: SmitheryServersResponse = client()
        ...
        .send()
        .map_err(|e| OacError::Network(format!("Failed to reach Smithery: {e}")))?
        .error_for_status()
        .map_err(|e| OacError::Network(format!("Smithery returned error: {e}")))?
        .json()
        .map_err(|e| OacError::Network(format!("Failed to parse Smithery response: {e}")))?;
    Ok(resp
        .servers
        .into_iter()
        .map(|s| MarketplaceItem {
...
pub fn trending_skills(limit: usize) -> Result<Vec<MarketplaceItem>, OacError> {
...
pub fn trending_servers(limit: usize) -> Result<Vec<MarketplaceItem>, OacError> {
    // Rotate through pages 1-5 based on day of year
    let page = (chrono::Utc::now().ordinal() % 5) + 1;
...
pub async fn trending_skills_async(limit: usize) -> Result<Vec<MarketplaceItem>, OacError> {
    let page = (chrono::Utc::now().ordinal() % 5) + 1;
    let url = format!("{SMITHERY_API}/skills?pageSize={}&page={}", limit, page);
...
pub async fn trending_servers_async(limit: usize) -> Result<Vec<MarketplaceItem>, OacError> {
    let page = (chrono::Utc::now().ordinal() % 5) + 1;
    let url = format!("{SMITHERY_API}/servers?pageSize={}&page={}", limit, page);
    let resp: SmitheryServersResponse = async_client()
        .get(&url)
...
pub fn fetch_audit_info(source: &str, skill_id: &str) -> Result<Option<SkillAuditInfo>, OacError> {
    let cache_key = format!("{source}/{skill_id}");
    ...
    let resp = client()
        ...
        .send()
        .map_err(|e| OacError::Network(format!("Failed to reach audit service: {e}")))?;
    let result = if resp.status().is_success() {
        match resp.json::<HashMap<String, SkillAuditInfo>>() {
            Ok(data) => data.into_values().next(),
            Err(e) => {
                eprintln!("[oac] warning: failed to parse audit response for {cache_key}: {e}");
                None
            }
        }
    } else {
...
pub async fn fetch_audit_info_async(
    source: &str,
    skill_id: &str,
) -> Result<Option<SkillAuditInfo>, OacError> {
    let cache_key = format!("{source}/{skill_id}");
    if let Ok(mut cache) = AUDIT_INFO_CACHE.lock()
        && let Some((ts, cached)) = cache.get(&cache_key)
        && ts.elapsed() < DETAIL_CACHE_TTL
...
pub async fn fetch_cli_readme_async(source: &str) -> Result<String, OacError> {
    ...
    Err(OacError::NotFound(format!("No README found for {source}")))
}
...
/// OAC-owned registry. Execution also resolves only from this embedded list.
...
fn fetch_github_stars(owner_repo: &str) -> Option<u64> {
    ...
    let stars = client()
        ...
        .header("User-Agent", "Open Agent Config")
        .send()
        .ok()
...
pub fn list_cli_registry() -> Vec<MarketplaceItem> {
    let registry = CLI_REGISTRY.clone();
...
/// Look up a CLI entry from the embedded OAC registry.
