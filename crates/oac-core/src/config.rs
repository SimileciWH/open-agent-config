...
impl Config {
    pub fn load(path: &Path) -> Result<Self, OacError> {
        if path.exists() {
    ...
    pub fn save(&self, path: &Path) -> Result<(), OacError> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)?;
        }
        let content = toml::to_string_pretty(self).map_err(|e| OacError::Internal(e.to_string()))?;
