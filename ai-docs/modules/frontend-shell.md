...
Frontend Shell 负责 Jira/Wiki 风格的蓝白工作区框架：深蓝侧栏、纸张感主内容、顶部工作区/作用域切换，以及右上角语言和外观快速偏好。独立 Settings 页面已删除，路径与 Agent 动作进入其工作上下文。
...
- ScopeSwitcher 从侧栏移到 Topbar，保留 All scopes、Global 和项目选择，并直接承载 Add Project 与 Manage Projects 弹窗。
- `#/settings` 仅保留兼容重定向到 Agents；侧栏不再显示 Settings。版本号位于侧栏底部的只读产品信息。
